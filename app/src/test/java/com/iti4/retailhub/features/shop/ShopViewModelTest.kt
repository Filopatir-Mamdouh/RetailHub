...
class ShopViewModelTest {
    ...
    @Test
    fun `getCategoriesList  (api state is success)`() = runTest {
        // init repo function.

        coEvery { repository.getProductTypesOfCollection() } returns flow {
            emit(
                listOf(
                    mockk<Category>(),
                    mockk<Category>()
    ...
    @Test
    fun `getCategoriesList, (returns throws an exception ) AND (api state is error)`() =
        runTest {
            coEvery { repository.getProductTypesOfCollection() } returns flow {
